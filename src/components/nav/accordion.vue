<template>
    <div class="accordion-item-dark">
        <button
            :class="{ 'collapsed': isCollapsed, 'accordion-button': true, 'sidenav-text': true}"
            type="button"
        ...
        >
            ...
            <div class="title-container sidenav-text">
                <router-link :to="`/${ title.toLowerCase() }`" v-if="!isNested" class="sidenav-text">
                    {{ title }}
                </router-link>
                <span v-else>
                    {{ title }}
            ...
            <div class="dropdown-icon-container sidenav-text">
                <PhCaretRight v-if="!isNested" :size="22" />
                <PhCaretDown v-else-if="!isCollapsed" :size="22" />
                <PhCaretUp v-else :size="22" />
            </div>
        </button>
        <div :id="`flush-collapse${ id }`" :class="`accordion-collapse collapse`" data-bs-parent="#accordionFlushExample" v-if="isNested">
            <div class="accordion-body">
                <span v-for="(item, index) in items" :key="index">
                    <router-link :to="item.path" class="nav-link accordion-text">
                        {{ item.name }}
                    </router-link>
                </span>
            </div>
...
<script setup>
import { PhGear, PhCaretDown, PhCaretUp, PhCaretRight } from "@phosphor-icons/vue";
