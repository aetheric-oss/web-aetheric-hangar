        ...
        <div class="popup-container business-popup">
            <div class="acc-popup-container">
                <div class="acc-popup-header">
                    ...
                    <div class="acc-popup-close-btn" @click="closeBusinessPopup">
                        <PhX size="22" color="white"/>
                    </div>
                </div>
                <div class="businesses-list">
                    <template v-for="(letterBusinesses, letter) in businesses" :key="letter">
                        <div class="letter sidenav-text">
                            {{ letter }}
                        </div>
                        <div class="businesses-card-list">
                            <hr>
                            <template v-for="business in letterBusinesses" :key="business">
                                <SidenavCard :cardText="business.name" :highlightBg="business.name.toLowerCase() === currentBusiness.name.toLowerCase()">
                                    <template #left-icon>
                                        <img :src="business.imgSrc" alt="Current Business Image" v-if="business.imgSrc">
                                    </template>
                                    <template #right-icon v-if="business.name.toLowerCase() === currentBusiness.name.toLowerCase()">
                                        <span class="highlight-text">Current</span>
                                    </template>
                                </SidenavCard>
                            </template>
                        </div>
                ...
                <div class="current-account-component">
                    <CurrentBusinessAccount :mildBackground="true" :business="currentBusiness"/>
                </div>
            </div>
        </div>
    </div>
